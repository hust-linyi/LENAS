...
class InputTransition(nn.Module):
    def __init__(self, outChans, elu):
        ...
        self.conv1 = nn.Conv3d(9, outChans, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        # print("out:", out.shape)
        # assert 1>3
        # out = self.relu1(torch.add(out, x16)) # lin commit
        out = self.relu1(out) # lin add
        # print(out.shape) # 1, 16, 16, 256, 256
        # assert 1>3
        return out
...
class OutputTransition(nn.Module):
    def __init__(self, inChans, elu, nll):
        ...
        if nll:
            # self.softmax = F.log_softmax # lin commit
