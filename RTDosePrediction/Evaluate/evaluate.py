...
def get_3D_Dose_dif_2(pred, gt, possible_dose_mask=None):
    if possible_dose_mask is not None:
        pred = pred[possible_dose_mask > 0]
        gt = gt[possible_dose_mask > 0]

    dif = np.mean(np.square(pred - gt))
    return dif
...
def get_Dose_score_and_DVH_score(prediction_dir, gt_dir, list_patient_ids):

    list_dose_dif = []
    list_dose_dif_2 = []
    list_structure_dose_dif = [[] for _ in range(10)]
    list_DVH_dif = []
    list_DVH_dif_2 = []
    
    # list_patient_ids = tqdm(os.listdir(prediction_dir))
    list_patient_ids = tqdm(list_patient_ids)
    result = {'D1':[], 'D95':[], 'D99':[], 'D_0.1_cc':[], 'mean':[]}
    for patient_id in list_patient_ids:
        ...
        list_dose_dif_2.append(get_3D_Dose_dif_2(pred, gt, possible_dose_mask))


        for i, structure_name in enumerate(['Brainstem',
                                            'SpinalCord',
                                            'RightParotid',
                                            'LeftParotid',
                                            'Esophagus',
                                            ...
                                            'PTV56']):
            try:
                structure_dose_mask_nii = sitk.ReadImage(gt_dir + '/' + patient_id + '/{}.nii.gz'.format(structure_name))
                structure_dose_mask = sitk.GetArrayFromImage(structure_dose_mask_nii)
                list_structure_dose_dif[i].append(get_3D_Dose_dif(pred, gt, structure_dose_mask))
            except:
                pass
        ...
        for structure_dose_dif in list_structure_dose_dif:
            # print(np.mean(structure_dose_dif), np.std(structure_dose_dif))
            ...
            for structure_name in ['Brainstem',
                ...
                if os.path.exists(structure_file):
                    ...
                    for metric in gt_DVH.keys():
                        ...
                        list_DVH_dif_2.append(np.square(gt_DVH[metric] - pred_DVH[metric]))
                        result[metric].append(abs(gt_DVH[metric] - pred_DVH[metric]))
                    
            
    #return np.mean(list_dose_dif), np.mean(list_DVH_dif)
    #print(np.mean(list_dose_dif), np.std(list_dose_dif), np.mean(list_dose_dif_2))
    #print(np.mean(list_DVH_dif), np.std(list_DVH_dif), np.mean(list_DVH_dif_2))

    # print(result)
    # print(np.mean(list_DVH_dif), np.std(list_DVH_dif))
    return np.mean(list_dose_dif), np.mean(list_DVH_dif)
