...
if __name__ == "__main__":
    if not os.path.exists('YOUR_ROOT/Data/OpenKBP_spacing'):
        raise Exception('OpenKBP_C3D should be prepared before testing, please run prepare_OpenKBP_spacing.py')

    parser = argparse.ArgumentParser()
    parser.add_argument('--GPU_id', type=int, default=0,
                        help='GPU id used for testing (default: 0)')
    parser.add_argument('--model_path', type=str, default='YOUR_ROOT/Experiment/RTDosePrediction/Output/C3D_spacing/best_val_evaluation_index.pkl')
    parser.add_argument('--TTA', type=bool, default=True,
                        help='do test-time augmentation, default True')
    args = parser.parse_args()

    trainer = NetworkTrainer()
    trainer.setting.project_name = 'C3D_spacing'
    trainer.setting.output_dir = 'YOUR_ROOT/Experiment/RTDosePrediction/Output/C3D_spacing'
    ...
    # Start inference
    print('\n\n# Start inference !')
    list_patient_dirs = ['YOUR_ROOT/Data/OpenKBP_spacing/pt_' + str(i) for i in range(241, 341)]
    ...
    Dose_score, DVH_score = get_Dose_score_and_DVH_score(prediction_dir=trainer.setting.output_dir + '/Prediction',
                                                         gt_dir='YOUR_ROOT/Data/OpenKBP_spacing')
