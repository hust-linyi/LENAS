...
if __name__ == "__main__":
    if not os.path.exists('YOUR_ROOT/Data/RTDosePrediction/OpenKBP_C3D'):
        raise Exception('OpenKBP_ResC3D should be prepared before testing, please run prepare_OpenKBP_ResC3D.py')

    parser = argparse.ArgumentParser()
    parser.add_argument('--GPU_id', type=int, default=0,
                        help='GPU id used for testing (default: 0)')
    parser.add_argument('--model_path', type=str, default='YOUR_ROOT/Experiment/RTDosePrediction/Output/ResC3D/best_val_evaluation_index.pkl')
    parser.add_argument('--TTA', type=bool, default=True,
                        help='do test-time augmentation, default True')
    args = parser.parse_args()

    trainer = NetworkTrainer()
    trainer.setting.project_name = 'ResC3D'
    trainer.setting.output_dir = 'YOUR_ROOT/Experiment/RTDosePrediction/Output/ResC3D'
