...
class NetworkTrainer:
    ...
    def set_GPU_device(self, list_GPU_ids):
        ...
        # multi-GPU
            ...
            self.setting.device = torch.device('cuda:' + str(list_GPU_ids[0]))
            self.setting.G_network = nn.DataParallel(self.setting.G_network, device_ids=list_GPU_ids)
            self.setting.D_network = nn.DataParallel(self.setting.D_network, device_ids=list_GPU_ids)
        self.setting.G_network.to(self.setting.device)
        self.setting.D_network.to(self.setting.device)
    ...
    def set_optimizer(self, optimizer_type, args):
        ...
        if optimizer_type == 'Adam':
            if hasattr(self.setting.G_network, 'decoder') and hasattr(self.setting.G_network, 'encoder'):
                self.setting.G_optimizer = optim.Adam([
                    {'params': self.setting.G_network.encoder.parameters(), 'lr': args['lr_encoder']},
                    {'params': self.setting.G_network.decoder.parameters(), 'lr': args['lr_decoder']}
                ],
                ...
                self.setting.G_optimizer = optim.Adam(self.setting.G_network.parameters(),
                                                    lr=args['lr'],
                                                    weight_decay=3e-5,
                                                    betas=(0.9, 0.999),
                                                    eps=1e-08,
                                                    amsgrad=True)
            self.setting.D_optimizer = optim.Adam([
                {'params': self.setting.D_network.parameters(), 'lr': args['lr']}
            ],
                weight_decay=args['weight_decay'],
                betas=(0.9, 0.999),
                eps=1e-08,
    ...
    def set_lr_scheduler(self, lr_scheduler_type, args):
        if lr_scheduler_type == 'step':
            ...
            self.setting.lr_scheduler = optim.lr_scheduler.MultiStepLR(self.setting.G_optimizer,
            ...
            self.setting.lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(self.setting.G_optimizer,
                                                                             T_max=args['T_max'],
            ...
            self.setting.lr_scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.setting.G_optimizer,
                                                                             mode='min',
                                                                             factor=args['factor'],
    ...
    def forward(self, input_, phase):
        ...
        # Forward
        if phase == 'train':
            self.setting.G_optimizer.zero_grad()
            self.setting.D_optimizer.zero_grad()
        output = self.setting.G_network(input_)

        return output

    def G_backward(self, output, target):
        time_start_load_data = time.time()
        ...
        # Optimize
        g_loss = self.setting.G_loss_function(output, target)
        g_loss.backward()

        self.setting.G_optimizer.step()

        return g_loss

    def D_backward(self, input_, real, fake, phase):
        time_start_load_data = time.time()
        ...
        for input_i in range(len(input_)):
            input_[input_i] = input_[input_i].to(self.setting.device)

        for real_i in range(len(real)):
            real[real_i] = real[real_i].to(self.setting.device)

        for fake_i in range(len(fake)):
            fake[fake_i] = fake[fake_i].to(self.setting.device)


        if phase == 'train':
            self.setting.D_optimizer.zero_grad()

        fake_AB = torch.cat((input_, fake), 1)
        pred_fake = self.setting.D_network(fake_AB.detach())
        d_loss_fake = self.setting.D_loss_function(pred_fake, False)

        real_AB = torch.cat((input_, real), 1)
        pred_real = self.setting.D_network(real_AB.detach())
        d_loss_real = self.setting.D_loss_function(pred_real, True)

        d_loss = (d_loss_fake + d_loss_real) * 0.5
        d_loss.backward()
        self.setting.D_optimizer.step()
    ...
    def train(self):
        ...
        self.setting.G_network.train()
        self.setting.D_network.train()
        sum_train_loss = 0.
        count_iter = 0
        ...
        for batch_idx, list_loader_output in enumerate(self.setting.train_loader):
            ...
            # Forward
            pred_fake = self.forward(input_, phase='train')
            
            
            d_loss = self.D_backward(input_, target, pred_fake, phase='train')
            sum_train_d_loss += d_loss.item()
            self.update_moving_train_loss(d_loss)
            
            g_loss = self.G_backward(input_, target, phase='train')
            sum_train_g_loss += g_loss.item()
            self.update_moving_train_loss(g_loss)
    ...
    def run(self):
        ...
        while (self.log.epoch < self.setting.max_epoch - 1) and (self.log.iter < self.setting.max_iter - 1):
            ...
            self.print_log_to_file('    Begin G_lr is %12.12f, %12.12f\n' % (
                self.setting.G_optimizer.param_groups[0]['lr'], self.setting.G_optimizer.param_groups[-1]['lr']), 'a')
            self.print_log_to_file('    Begin D_lr is %12.12f, %12.12f\n' % (
                self.setting.D_optimizer.param_groups[0]['lr'], self.setting.D_optimizer.param_groups[-1]['lr']), 'a')
            # Record initial learning rate for this epoch
            self.log.list_lr_associate_iter.append([self.setting.G_optimizer.param_groups[0]['lr'], self.log.iter])
            self.log.list_lr_associate_iter.append([self.setting.D_optimizer.param_groups[0]['lr'], self.log.iter])
            ...
            self.print_log_to_file('    End G_lr is %12.12f, %12.12f\n' % (
                self.setting.G_optimizer.param_groups[0]['lr'], self.setting.G_optimizer.param_groups[-1]['lr']), 'a')
            self.print_log_to_file('    End D_lr is %12.12f, %12.12f\n' % (
                self.setting.D_optimizer.param_groups[0]['lr'], self.setting.D_optimizer.param_groups[-1]['lr']), 'a')
    ...
    def save_trainer(self, status='latest'):
        if len(self.setting.list_GPU_ids) > 1:
            G_network_state_dict = self.setting.G_network.module.state_dict()
            D_network_state_dict = self.setting.D_network.module.state_dict()
        else:
            G_network_state_dict = self.setting.G_network.state_dict()
            D_network_state_dict = self.setting.D_network.state_dict()

        G_optimizer_state_dict = self.setting.G_optimizer.state_dict()
        D_optimizer_state_dict = self.setting.D_optimizer.state_dict()
        lr_scheduler_state_dict = self.setting.lr_scheduler.state_dict()

        ckpt = {
            'G_network_state_dict': G_network_state_dict,
            'D_network_state_dict': D_network_state_dict, 
            'lr_scheduler_state_dict': lr_scheduler_state_dict,
            'G_optimizer_state_dict': G_optimizer_state_dict,
    ...
    def init_trainer(self, ckpt_file, list_GPU_ids, only_network=True):
        ...
        self.setting.G_network.load_state_dict(ckpt['G_network_state_dict'])
        self.setting.D_network.load_state_dict(ckpt['D_network_state_dict'])

        if not only_network:
            self.setting.lr_scheduler.load_state_dict(ckpt['lr_scheduler_state_dict'])
            self.setting.G_optimizer.load_state_dict(ckpt['G_optimizer_state_dict'])
            self.setting.D_optimizer.load_state_dict(ckpt['D_optimizer_state_dict'])
            self.log = ckpt['log']
        ...
        # This for Adam
        if type(self.setting.G_optimizer).__name__ == 'Adam':
            for key in self.setting.G_optimizer.state.items():
