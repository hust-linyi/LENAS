...
class TrainerSetting:
    def __init__(self):
        ...
        self.loss_function2 = None
        self.loss_function3 = None
...
class NetworkTrainer:
    ...
    def backward(self, output, target):
        ...
        l1_loss = self.setting.loss_function1(output, target)

        # print(l1_loss)

        between_loss = self.setting.loss_function2(output, target) * 0.05

        d_loss = self.setting.loss_function3(output, target) * 0.01
        
        print(l1_loss, between_loss, d_loss)

        # print(l1_loss, ce_loss)
        loss = l1_loss + between_loss + d_loss
        # loss = l1_loss
        # loss = d_loss
    ...
    def train(self):
        ...
        for batch_idx, list_loader_output in enumerate(self.setting.train_loader):
            ...
            teacher_ = [torch.zeros_like(target[0]).cuda(), [0. for _ in range(len_out_dims)]]
            for teacher in teachers:
                with torch.no_grad():

                    out0, out1 = teacher(input_)

                    # del model
                    # gc.collect()

                    teacher_[0] += out0
                    for i in range(len_out_dims):
                        teacher_[1][i] += out1[i]
            
            teacher_[0] /= len(teachers)
            for i in range(len_out_dims):
                teacher_[1][i] /= len(teachers)

            target[0] = teacher_[0]
            target.append(teacher_[1])
            ...
            #     print(output[1][i].shape, target[2][i].shape)
